"""
    In this experiment, we measure the number of simulation steps required to fully open and close the gripper when there is a sphere between the jaws.
"""
    ...
    while True:
        if not gripper_moved:   # Move the gripper
            if not np.linalg.norm(robot.get_tcp_pose()[0] - position) < 1e-4:
                ...
                create_sphere(radius=radius, position=position, mass=0.1)
                gripper_moved = True
        elif not gripper_closed:  # Close the gripper
            if not robot._is_gripper_closed(close_threshold=radius) or not robot._is_gripper_stopped():
                robot.close_gripper()
                close_count += 1
            else:
                gripper_closed = True
