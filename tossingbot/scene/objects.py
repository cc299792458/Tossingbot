def create_box(p, position=[0, 0, 1], orientation=[0, 0, 0], half_extents=[0.5, 0.5, 0.5], mass=1):
    """
    Function to create a box in the PyBullet simulation.

    :param p: PyBullet instance (usually pybullet module)
...
def create_sphere(p, radius=0.5, position=[0, 0, 1], mass=1):
    """
...
def create_capsule(p, radius=0.2, height=1.0, position=[0, 0, 1], orientation=[0, 0, 0], mass=1):
    """
...
def create_cylinder(p, radius=0.3, height=1.0, position=[0, 0, 1], orientation=[0, 0, 0], mass=1):
    """
...
def create_plane(p, position=[0, 0, 0], orientation=[0, 0, 0], mass=0):
    """
...
def create_mesh(p, mesh_file, position=[0, 0, 1], orientation=[0, 0, 0], mass=1):
    """
...
if __name__ == '__main__':
    ...
    plane_id = create_plane(p, mass=0)  # Fixed plane
    box_id = create_box(p, position=[1, 1, 1], half_extents=[0.5, 0.5, 0.5], mass=0)  # Fixed box
    sphere_id = create_sphere(p, position=[-1, 1, 1], radius=0.5, mass=1)  # Movable sphere
    capsule_id = create_capsule(p, position=[2, 1, 1], radius=0.2, height=1.0, mass=1)  # Fixed capsule
    cylinder_id = create_cylinder(p, position=[-2, 1, 1], radius=0.3, height=1.0, mass=0)  # Movable cylinder

    # Example: Load a mesh from an .obj file
    for i in range(8):
        if i != 5:
            mesh_file = f'assets/meshes/objects/blocks/{i}.obj'
            mesh_id = create_mesh(p, mesh_file=mesh_file, position=[0, -1, 0.3])  # Replace with path to mesh file
